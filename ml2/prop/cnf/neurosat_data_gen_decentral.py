"""Decentralized NeuroSAT data generation"""
...
@ray.remote
def neurosat_data_gen_worker_fn(
    server: DataServer,
    batch_size: int,
    min_n: int,
    max_n: int,
    ...
    while not ray.get(server.finished.remote()):
        ...
        while len(batch) < batch_size:
            if problem is None:
                problem = NeuroSatProblem.from_random(min_n=min_n, max_n=max_n, timeout=timeout)
