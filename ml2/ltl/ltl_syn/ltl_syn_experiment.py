...
class LTLSynExperiment(Seq2SeqExperiment):
    ...
    def analyze_eval_file(self, file: str):
        ...
        with open(file, 'r') as eval_file:
            ...
            for row in csv_reader:
                ...
                if not target:
                    continue
                _, _, num_latches, _, _ = header_ints_from_str(
                    target.replace('\\n', '\n'))
                if num_latches not in stats:
                    stats[num_latches] = {}
                status = row['status']
                stats[num_latches][status] = stats[num_latches].get(status,
                                                                    0) + 1
        print(stats)

    def analyze_eval_circuits(self,
                              directory: str,
                              buckets: list = None,
                              match: bool = False,
        ...
        with open(os.path.join(directory, 'log.csv'), 'r') as eval_file:
            ...
            for row in csv.DictReader(eval_file):
                ...
                max_var_id, num_inputs, num_latches, num_outputs, num_and_gates = header_ints_from_str(
                    target.replace('\\n', '\n'))
    ...
    def plot_eval_stats(self,
        ...
        perc = [
            '%.2f' % (x / (x + y))
            for (x, y) in zip(match_or_satisfied, violated)
        ]
    ...
    def analyze_eval_file_realizability(self, file: str):
        stats = {
            'Realizable': {
        ...
        with open(file, 'r') as eval_file:
            ...
            for row in csv_reader:
                realizable = 'Realizable' if 'i0 i0' in row[
                    'target'] else 'Unrealizable'
                status = row['status']
        ...
        for alpha in alphas:
            ...
            for beam_size in beam_sizes:
                ...
                logger.info(
                    ...
                    steps, self.alpha, self.beam_size, self.batch_size)
                ...
                logger.info(
                    ...
                    self.alpha, self.beam_size, self.batch_size)
    ...
    def eval_ltl_specs(self,
        ...
        spec_ds = LTLSpecData.from_bosy_files(LTLSpecData.local_path(name),
                                              spec_filter)
    ...
    def eval_generator(self,
        ...
        def eval_batch():
            ...
            batch = next(
                iter(batch_dataset.batch(self.batch_size,
        ...
        with open(stats_filepath, 'w') as stats_file:
            json.dump(counters, stats_file, indent=4)
    ...
    def add_init_args(cls, parser):
        ...
        parser.add_argument('--no-encode-realizable',
                            action='store_false',
                            dest='encode_realizable')
