...
from ...data.stats import plot_stats, write_stats
...
from .ltl_syn_data_gen_common import check_lower_bounds, check_upper_bounds, csv_dataset_writer, csv_file_writer, add_ltl_syn_data_gen_args, progress_bar, ProgressActor
...
def translate_substrings(string: str, table: dict):
    if not table:
        return string
    return re.sub('|'.join(re.escape(original) for original in table),
                  lambda k: table[k.group(0)], string)
...
class DataSetActor:
    ...
    def add_assumption(self, problem):
            ...
            while True:
                try:
                    resampled_inputs = np.random.choice(
                        problem['inputs'],
                        len(assumption['inputs']),
                        replace=False).tolist()
                    resampled_outputs = np.random.choice(
                        problem['outputs'],
                        len(assumption['outputs']),
                        replace=False).tolist()
                except ValueError:
                    choices.remove(idx)
                    if choices:
                ...
                translation_table = dict(
                    zip(assumption['inputs'] + assumption['outputs'],
    ...
    def add_guarantee(self, problem):
        ...
        if self.params['resample_aps']:
            ...
            translation_table = dict(
                zip(guarantee['inputs'] + guarantee['outputs'],
                    resampled_inputs + resampled_outputs))
            guarantee['pattern'] = translate_substrings(guarantee['pattern'],
                                                        translation_table)
    ...
    def post_solved_problems(self, problems):
        ...
        for problem in problems:
            ...
            if status == LTLSynStatus.REALIZABLE and (
            ...
            elif status == LTLSynStatus.UNREALIZABLE and (
                    not max_num_assumptions or
                    num_assumptions < max_num_assumptions) and (
                        not max_num_assumption_trials or
                        num_assumption_trials < max_num_assumption_trials):
                self.open.append(problem)
            elif status == LTLSynStatus.UNREALIZABLE or status == LTLSynStatus.TIMEOUT or (
                    status == LTLSynStatus.REALIZABLE and
                    num_guarantees == max_num_guarantees):
                ...
                if choose_realizable and status in (LTLSynStatus.UNREALIZABLE,
                ...
                try:
                    violated_bounds = check_upper_bounds(
...
def main(args):
    ...
    ds_actor = DataSetActor.remote(guarantees, args.inputs, args.outputs,
                                   progress_actor, args_dict, samples_queue,
                                   timeouts_queue, assumptions)
    ...
    timeouts_writer_result = csv_file_writer.remote(timeouts_queue,
                                                    timeouts_file)
    if args.tool == 'bosy':
        worker_results = [
            bosy_worker_fn.remote(ds_actor,
                                  id=i,
    ...
    split_dataset = LTLSynSplitData.from_dir(args.name)
    ...
    split_dataset.save(folder_path, upload=args.upload)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=
        'Generates a synthesis dataset from a set of assumption patterns and a set of guarantee patterns'
    )
    ...
    parser.add_argument('-p',
                        ...
                        default='scp-0',
                        metavar='name',
                        help=('name of patterns'))
    parser.add_argument('--no-unique-assumptions',
                        action='store_false',
                        dest='unique_assumptions',
                        help=('assumptions in a single sample are not '
                              'necessarily unique'))
    ...
    parser.add_argument('--max-assumption-trials',
                        type=int,
                        default=None,
                        metavar='max',
                        help=('maximum number of trials to find an assumption'
