        ...
        for _, row in self.data_frame.iterrows():
            sample = {
                'assumptions':
                    ...
                    if 'assumptions' in row and row['assumptions'] else
...
class LTLSynSplitData(SplitSupervisedData):
    ...
    def plot_stats(self, splits: list = None):
        ...
        filepath = os.path.join(self.stats_path(self.name),
                                'max_num_latches.eps')
...
def space_dataset(path: str, num_next: int = 2):
    ...
    for split in ['train', 'val', 'test', 'timeouts']:
        ...
        df['guarantees'] = df['guarantees'].str.replace(fin_regex,
                                                        r'\g<1> \g<2>')
        df['guarantees'] = df['guarantees'].str.replace(glob_regex,
                                                        r'\g<1> \g<2>')
        for _ in range(num_next):
            df['guarantees'] = df['guarantees'].str.replace(
                next_regex, r'\g<1> \g<2>')
    ...
    for split in ['train', 'val', 'test']:
        with tqdm(desc=split) as pbar:
            for sample in split_dataset[split].sample_generator():
                result = nuxmv.model_check(LTLSpec.from_dict(sample),
                                           sample['circuit'] + '\n',
                                           realizable=sample['realizable'],
                                           timeout=timeout)
