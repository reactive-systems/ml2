...
class LTLSpecPatternData(Data):
    ...
    def __init__(self,
        ...
        logger.info(
            ...
            len(self.guarantees), len(self.assumptions))

    def filter(self,
               max_ast_size: int,
               max_num_inputs: int,
               max_num_outputs: int,
               type: str = 'guarantees') -> None:
        if type == 'guarantees':
        ...
        logger.info(
            ...
            type, counter['max_ast_size'], counter['num_inputs'],
            counter['num_outputs'])

    def rename(self,
               inputs: list,
               outputs: list,
               shuffle_aps: bool = None,
               type: str = 'guarantees') -> None:
        if type == 'guarantees':
            props = self.guarantees
        elif type == 'assumptions':
        ...
        for prop in props:
            ...
            if shuffle_aps:
                renamed_inputs = list(
                    random.choice(inputs, num_inputs, replace=False))
                renamed_outputs = list(
                    random.choice(outputs, num_outputs, replace=False))
            else:
                renamed_inputs = inputs[:num_inputs]
                renamed_outputs = outputs[:num_outputs]
            renaming = dict(
                zip(prop['inputs'] + prop['outputs'],
    ...
    def save_to_path(self, path: str) -> None:
        ...
        with open(assumptions_path, 'w') as assumptions_file:
            json.dump({'patterns': self.assumptions},
                      assumptions_file,
                      indent=2)
            logger.info('Written %d assumption patterns to %s',
                        len(self.assumptions), assumptions_path)
        ...
        with open(guarantees_path, 'w') as guarantees_file:
            ...
            logger.info('Written %d guarantee patterns to %s',
                        len(self.guarantees), guarantees_path)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=
        'Extracts assumption and guarantee patterns from LTL specification data and writes them to a file'
    )
    ...
    parser.add_argument('--max-ast-size',
                        type=int,
                        default=None,
                        help=('maximum size of the abstract syntax tree'
                              'representing the pattern'))
    parser.add_argument('--shuffle-aps',
                        action='store_true',
                        help='shuffle atomic propositions when renaming')
    parser.add_argument('--ltl-spec-data',
                        default='sc-0',
                        help='LTL specification data')
    parser.add_argument(
        '--unique-properties',
        action='store_true',
    ...
    parser.add_argument(
        '--name',
        default='scp',
        help='name of the extracted LTL specification pattern dataset')
    ...
    ltl_spec_pattern_data.filter(args.max_ast_size, len(args.inputs),
                                 len(args.outputs), 'guarantees')
    ltl_spec_pattern_data.filter(args.max_ast_size, len(args.inputs),
                                 len(args.outputs), 'assumptions')
    ltl_spec_pattern_data.rename(args.inputs, args.outputs, args.shuffle_aps,
                                 'guarantees')
    ltl_spec_pattern_data.rename(args.inputs, args.outputs, args.shuffle_aps,
                                 'assumptions')
