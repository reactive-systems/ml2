...
class KAryPattern(Pattern):
    ...
    def filled_childs(self, inputs, outputs):
        ...
        for child in self.childs:
            filled_childs.append(
                child.fill(inputs[input_index:], outputs[output_index:]))
    ...
    def fill(self, inputs, outputs):
        ...
        if self.n > 1:
            if len(filled_child) > 2:
                filled_child = f'( {filled_child} )'
            return ' & '.join(
                [f'{"X " * i}{filled_child}' for i in range(self.n)])
        return filled_child
...
class Eventually(KAryPattern):
    ...
    def fill(self, inputs, outputs):
        ...
        for child in self.childs:
            ...
            output_index += child.num_outputs
        return ' & '.join(
            [f'F ( {filled_child} )' for filled_child in filled_childs])
...
class Globally(KAryPattern):
    ...
    def fill(self, inputs, outputs):
        ...
        for child in self.childs:
            ...
            output_index += child.num_outputs
        return ' | '.join(
            [f'G ( {filled_child} )' for filled_child in filled_childs])
...
class CNFMutualExclusion(KAryPattern):
    ...
    def fill(self, inputs, outputs):
        filled_childs = [
            f'! {filled_child}'
            for filled_child in self.filled_childs(inputs, outputs)
        ]
        return ' & '.join([
