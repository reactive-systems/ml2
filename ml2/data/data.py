...
from ..globals import ML2_BUCKET, WANDB_ENTITY
    ...
    def tf_generator(self):
        for sample in self.generator():
            if not self.encoder.encode(sample):
                ...
                self.encoder_errors[error] = self.encoder_errors.get(error,
                                                                     0) + 1
                continue
...
class SplitUnsupervisedData(SplitData):
    ...
    def encoder_errors(self):
        return {
            name: split.encoder_errors for name, split in self._splits.items()
        }
