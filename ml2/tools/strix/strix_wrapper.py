...
def strix_wrapper_dict(problem: dict,
                       minimize_aiger=False,
                       minimize_mealy=False,
                       threads=None,
                       timeout=None):
    ltl_spec = LTLSpec.from_dict(problem)
    return strix_wrapper_str(ltl_spec.formula_str, ltl_spec.input_str,
                             ltl_spec.output_str, minimize_aiger,
                             minimize_mealy, threads, timeout)


def strix_wrapper_str(formula_str,
                      ins_str,
                      outs_str,
                      minimize_aiger=False,
                      minimize_mealy=False,
