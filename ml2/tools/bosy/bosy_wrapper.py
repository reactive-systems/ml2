...
def bosy_wrapper_dict(problem: dict,
                      bosy_path,
                      timeout,
                      optimize=False,
                      temp_dir='/tmp'):
    problem_str = format_bosy_input(problem['guarantees'], problem['inputs'],
                                    problem['outputs'],
...
def bosy_wrapper_str(problem_str,
                     bosy_path,
                     timeout,
                     optimize=False,
                     temp_dir='/tmp'):
...
def add_bosy_args(parser):
    parser.add_argument('--bosy-path',
                        default='/bosy/bosy.sh',
                        metavar='path',
                        help='Path to BoSy shell script')
    parser.add_argument('--bosy-timeout',
                        type=float,
                        default=10.0,
                        metavar='timeout',
                        help='BoSy timeout')
    parser.add_argument('--bosy-temp-dir',
                        default='/tmp',
                        metavar='dir',
                        help='Temporary BoSy directory')
