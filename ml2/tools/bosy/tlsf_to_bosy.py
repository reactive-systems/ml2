...
def tlsf_to_bosy(tlsf_dir, bosy_dir=None):
    ...
    for file in os.listdir(tlsf_dir):
        if file.endswith('.tlsf'):
            ...
            filepath_bosy = os.path.abspath(
                os.path.join(bosy_dir, filename + '.json'))

            try:
                subprocess.run(
                    ['syfco', '-f', 'bosy', '-o', filepath_bosy, filepath_tlsf])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        'Converts all TLSF files in the specified directory to JSON files that are conform with the BoSy input format using SyFCo'
    )
    parser.add_argument('--dir-tlsf',
                        type=str,
                        default=None,
                        help='Directory with TLSF files')
    parser.add_argument('--dir-bosy',
                        type=str,
                        default=None,
                        help='Directory where BoSy input files are written')
