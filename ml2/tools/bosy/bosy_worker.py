...
def bosy_worker(server, bosy_path, bosy_timeout, temp_dir):
    ...
    while ray.get(server.has_unsolved_problems.remote()):
        ...
        for problem in problems:
            solution = bosy_wrapper_dict(problem, bosy_path, bosy_timeout,
                                         temp_dir)
...
def add_parser_args(parser):
    parser.add_argument('--bosy-timeout',
                        ...
                        metavar='timeout',
                        help='BoSy timeout')
    parser.add_argument('--bosy-optimize',
                        action='store_true',
                        help='Optimize AIGER w.r.t. number of AND gates')