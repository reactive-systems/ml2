"""Keras trainer with distributed data parallelism (DDP)"""
...
class KerasTrainerDDP(Trainer):
    def __init__(
        ...
        batch_size_per_replica: int = None,
        ...
        **kwargs,
        ...
        assert drop_batch_remainder  # Forced because of data parallelism
    ...
    def get_tf_dataset(self, dataset: Dataset) -> tf.data.Dataset:
        ...
        tf_dataset = tf_dataset.batch(
            self.global_batch_size, drop_remainder=self.drop_batch_remainder
        )
        tf_dataset = tf_dataset.prefetch(2)
        return tf_dataset

    def train(self):
        assert (self.global_batch_size is None and self.batch_size_per_replica is not None) or (
            self.global_batch_size is not None and self.batch_size_per_replica is None
        )
        ...
        if self.batch_size_per_replica is None:
            self.batch_size_per_replica = int(
                self.global_batch_size / strategy.num_replicas_in_sync
            )
        if self.global_batch_size is None:
            self.global_batch_size = self.batch_size_per_replica * strategy.num_replicas_in_sync

        logger.info(
            "Number of devices: {}, Batch size per replica: {}, Global batch size: {}".format(
                strategy.num_replicas_in_sync, self.batch_size_per_replica, self.global_batch_size
            )
        )
        self.pipeline.num_replica = strategy.num_replicas_in_sync
        ...
        tf_train_data = tf_train_data.with_options(options)
        tf_val_data = tf_val_data.with_options(options)
