...
def create_model(params, training, custom_pos_enc=False, attn_weights=False):
    ...
    model_inputs = [input]
    if custom_pos_enc:
        positional_encoding = tf.keras.layers.Input((None, None, None),
                                                    dtype=tf.float32,
    ...
    if training:
        ...
        predictions = transformer.TransformerMetricsLayer(params)(
            [predictions, target])
...
class HierarchicalTransformer(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        input_padding_mask = tf.cast(
            tf.math.equal(input, self.params['input_pad_id']),
        ...
        input_padding_mask = tf.reshape(
            input_padding_mask,
            [batch_size, 1, 1, input_length_d0 * input_length_d1])
    ...
    def encode(self, inputs, padding_mask, positional_encoding, training):
        ...
        input_embedding_d1 = tf.reshape(
            input_embedding,
            [batch_size * input_length_d0, input_length_d1, d_embed_enc])
        ...
        return encoder_output_d0, attn_weights_d0
